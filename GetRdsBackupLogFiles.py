...
def parse_args(AccessKeyId, AccessKeySecret):
    # 定义默认起止时间
    start_time = (date.today() + timedelta(days=-7)).strftime("%Y-%m-%dT00:00:00Z")
    end_time = datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")

    # 解析入参
    parser = argparse.ArgumentParser(usage='%(prog)s [options]')
    parser.add_argument('-i', metavar='--instanceid', help='DB InstanceId')
    parser.add_argument('-st', default=start_time, metavar='--start-time', help='Backup Start Time.default now()-7 days. eg:2019-09-10T00:00:00Z')
    parser.add_argument('-et', default=end_time, metavar='--end-time', help='Backup End Time.default now(). eg:2019-09-10T00:00:00Z')
...
def downbakupfile(AccessKeyId, AccessKeySecret, args):
    ...
    for page in range(1, pages + 1):
        ...
        response = client.do_action_with_exception(request)
        backupdetail = eval(str(response, encoding='utf-8'))['Items']['BinLogFile']

        for i in range(len(backupdetail)):
            # bakfile_url = backupdetail[i]['BackupIntranetDownloadURL']          # 内网下载地址
            bakfile_url = backupdetail[i]['DownloadLink']                         # 外网下载地址
            re_result = str(backupdetail[i]['HostInstanceID']) + '_' + backupdetail[i]['LogFileName']
    ...
    for eachfile in os.listdir(path):
        ...
        if os.path.isfile(filename):
            ...
            if endfiletime > lastmodifytime:
                if re.findall('mysql-bin', filename)[0] == 'mysql-bin':
                    os.remove(filename)
